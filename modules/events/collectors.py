...
from vkbottle import Keyboard, KeyboardButtonColor, Text, Callback, GroupEventType, GroupTypes, Bot, API, LoopWrapper
    ...
    await message.answer(
        ...
        keyboard=(
            Keyboard(one_time=True, inline=False)
                ...
                .row()
                .add(Text("ℹ Когда начнется мероприятие", {"cmd": "collectors.info1"}), color=KeyboardButtonColor.SECONDARY)
                .row()
                .add(Text("ℹ Что надо делать на данном мероприятии?", {"cmd": "collectors.info2"}), color=KeyboardButtonColor.SECONDARY)
                .row()
                .add(Text("ℹ Сколько людей на мероприятии?", {"cmd": "collectors.info3"}), color=KeyboardButtonColor.SECONDARY)
                .row()
...
async def Buy(message: Message, bot: Bot, api: API):
    await database.setUserData(message.from_id, 'state', "'collectors.Buy'")
...
async def end1(message: Message, bot: Bot, api: API):
    ...
    await bot.api.messages.remove_chat_user(chat_id=event_data[5], user_id=message.from_id)
    await message.answer(
...
async def Menu(message: Message, bot: Bot, api: API):
    ...
    if data[79] <= int(time.time()):
        ...
        event_data = await database.getBdData('event', 'id', "'0'")
        await message.answer(
            message=f"🥚 Мероприятие собиратели » ⏰ {data[79] - int(time.time())} сек.\n\n"
            ...
            keyboard=(
                Keyboard(one_time=True, inline=False)
                    ...
                    .row()
                    .add(Callback("💬 Беседа мероприятия", payload={"cmd": "collectors.beseda"}), color=KeyboardButtonColor.SECONDARY)
            )
        )
